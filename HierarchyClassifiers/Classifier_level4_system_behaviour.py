...
def second_level_classifier(X, XNoNPL, Z, T, R, Ypred, Xvalidate, XvalidateNoNPL, Yvalidate, Zvalidate, Tvalidate, Rvalidate, level):
    ...
    trainLocations = get_relevant_section(Ztrain, 'behaviour')
    testLocations = get_relevant_section(Ztest, 'behaviour')
    valLocations = get_relevant_section(Zval, 'behaviour')
    ...
    third_level_classifier(XtrainSection+XtestSection, XtrainNoNPLSection+XtestNoNPLSection, TtrainSection+TtestSection, RtrainSection+RtestSection, valPredSection, ZvalSection, XvalSection, XvalNoNPLSection, TvalSection, RvalSection, 'behaviour')
...
def third_level_classifier(X, XNoNPL, T, R, Zpred, Zvalidate, Xvalidate, XvalidateNoNPL, Tvalidate, Rvalidate, level):
    ...
    Inputs: X: all sentences to train on
            ...
            Tvalidate: 3rd level true label of the validation sentences
            Rvalidate: 4th level true label of the validation sentences (labels to predict)
            level: indicates which sub-level we are working with, e.g. "requirements", "system", etc
    Returns: 
    """
    ...
    for sent in correctlyClassifiedSentences:
        Xval.append(Xvalidate[sent])
        XvalNoNPL.append(XvalidateNoNPL[sent])
        Tval.append(Rvalidate[sent])
        Rval.append(Rvalidate[sent])

    Xtrain, Xtest, XtrainNoNPL, XtestNoNPL, Ttrain, Ttest, Rtrain, Rtest = train_test_split(X, XNoNPL, R, R, test_size=0.25, random_state=42)

    print('')
    print("---------- BEHAVIOUR CLASSIFIER ----------")
    print('')
    print('Number of training instances: ', len(Xtrain))
    print('Number of testing instances: ', len(Xtest))
    print('Number of validation instances: ', len(Xval))
    ...
    pipeline = make_pipeline(
            ...
            #ExtraTreesClassifier(max_features=20, class_weight='balanced',min_impurity_split=0.6, random_state=42)
    ...
    labels=['b-techsolution','b-general']
    print('Total precision & recall (validation):')
...
#    with open('resBImplementation2.csv', mode='w') as res_file:
