...
def second_level_classifier(X, XNoNPL, Z, T, R, Ypred, Xvalidate, XvalidateNoNPL, Yvalidate, Zvalidate, Tvalidate, Rvalidate, level):
    ...
    print('')
    print("---------- DEVELOPMENT PROCESS CLASSIFIER ----------")
    print('')
    ...
    labels=['development practice','QA']
...
#        with open('resDevPractice.csv', mode='w') as res_file:
#            res_writer = csv.writer(res_file, delimiter='|', quotechar='"', quoting=csv.QUOTE_MINIMAL)
#            for i, inst in enumerate(ZtrainPred):
#                if ZtrainPred[i] == 'development practice':
#                    res_writer.writerow([XtrainNoNPL[i], ZtrainPred[i]])
#            for i, inst in enumerate(Zpred):
#                if Zpred[i] == 'development practice':
#                    res_writer.writerow([XtestNoNPL[i], Zpred[i]])
#            for i, inst in enumerate(valPred):
#                if valPred[i] == 'development practice':
#                    res_writer.writerow([XvalNoNPL[i], valPred[i]])

        
    trainLocations = get_relevant_section(Ztrain, 'QA')
    testLocations = get_relevant_section(Ztest, 'QA')
    valLocations = get_relevant_section(Zval, 'QA')
    ...
    third_level_classifier(XtrainSection+XtestSection, XtrainNoNPLSection+XtestNoNPLSection, TtrainSection+TtestSection, valPredSection, ZvalSection, XvalSection, XvalNoNPLSection, TvalSection, 'QA')
...
def third_level_classifier(X, XNoNPL, T, Zpred, Zvalidate, Xvalidate, XvalidateNoNPL, Tvalidate, level):
    ...
    print('')
    print("---------- QUALITY ASSURANCE CLASSIFIER ----------")
    print('')
    print('Number of training instances: ', len(Xtrain))
    print('Number of testing instances: ', len(Xtest))
    print('Number of validation instances: ', len(Xval))
...
def train_classifier():
    ...
    trainLocations = get_relevant_section(Ytrain, 'development process')
    testLocations = get_relevant_section(Ytest, 'development process')
    valLocations = get_relevant_section(Yval, 'development process')
    ...
    second_level_classifier(XtrainSection+XtestSection, XtrainNoNPLSection+XtestNoNPLSection, ZtrainSection+ZtestSection, TtrainSection+TtestSection, RtrainSection+RtestSection, valPredSection, XvalSection, XvalNoNPLSection, YvalSection, ZvalSection, TvalSection, RvalSection, 'development process')
